...
function getNextFriday(date) {
  ...
  while (nextFriday.getDay() !== fridayIndex) {
    nextFriday.setDate(nextFriday.getDate() + 1);
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const msInDayCount = 86400000;

  return (end - start) / msInDayCount + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return startDate <= newDate && newDate <= endDate;
}
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil((days + startDay) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      const year = startDate.getFullYear().toString().padStart(2, 0);
